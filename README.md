...
| filter_tables       | False    | None    | If an array of table or view names is provided, the tap will only reflect and process those objects. Names may be unqualified or use the `schema.table` format. If left blank, the tap automatically processes ALL available objects in the selected schemas. |
